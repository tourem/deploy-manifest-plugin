...
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.generateHtml=true
```
...
Les fichiers sont écrits à la racine du projet (ou dans `target/` selon configuration) : `deployment-manifest-report.json` et, en option, `deployment-manifest-report.yaml` / `deployment-manifest-report.html` / archive.
...
```bash
# Analyse simple
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:analyze-dependencies
...
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:dependency-report
...
- 🏥 **Statut de santé des dépendances** (NOUVEAU v2.7.0) :
  ...
  - Critères d'évaluation :
    ...
    - Nombre de contributeurs : WARNING si <3, DANGER si <2
    - Dépôt archivé : DANGER automatique
    - Mise à jour récente (<180 jours) : indicateur positif
...
- 🔌 **Analyse des plugins Maven** (NOUVEAU v2.7.0) :
  ...
  - Section Plugin Management avec indicateur "Utilisé dans le Build"
- 🎨 Thème sombre/clair avec toggle, responsive, professionnel

**Intégration CI/CD** :
```yaml
# GitHub Actions
- name: Analyze Dependencies
  run: mvn io.github.tourem:deploy-manifest-plugin:2.8.0:analyze-dependencies
...
JSON + YAML + HTML :
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.exportFormat=both -Ddescriptor.generateHtml=true
```

Arbre des dependances (Tree+Flat) avec scopes compile+runtime :
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.includeDependencyTree=true -Ddescriptor.dependencyTreeFormat=both -Ddescriptor.dependencyScopes=compile,runtime
```

Licences avec avertissements et liste incompatible personnalisee :
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.licenseWarnings=true -Ddescriptor.incompatibleLicenses=GPL-3.0,AGPL-3.0,SSPL
```

Proprietes incluant les variables d’environnement :
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.includeProperties=true -Ddescriptor.includeEnvironmentVariables=true
```

Plugins avec configuration et verification des mises a jour :
```bash
mvn io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.includePlugins=true -Ddescriptor.includePluginConfiguration=true -Ddescriptor.checkPluginUpdates=true -Ddescriptor.generateHtml=true
```
...
Le plugin peut créer des archives (ZIP, TAR.GZ, TAR.BZ2) contenant tous les rapports générés.

#### Archive Basique (Descriptor uniquement)

Par défaut, les archives contiennent uniquement les fichiers du descriptor :
...
#### Archive Complète (Tous les Rapports)

Utilisez `includeAllReports=true` pour inclure **tous les rapports générés** dans l'archive :
...
| Format | Extension | Compression | Cas d'Usage |
|--------|-----------|-------------|-------------|
| `zip` | `.zip` | DEFLATE | Universel, compatible Windows |
| `tar.gz` / `tgz` | `.tar.gz` | GZIP | Standard Linux/Unix |
| `tar.bz2` / `tbz2` | `.tar.bz2` | BZIP2 | Meilleure compression |
| `jar` | `.zip` | DEFLATE | Écosystème Java |

#### Déployer l'Archive dans un Repository Maven
...
**Résultat** : Archive déployée en tant que `myapp-1.0.0-docs.zip` dans le repository Maven
...
- name: Generate Deploy Manifest
  run: mvn -B io.github.tourem:deploy-manifest-plugin:2.8.0:generate -Ddescriptor.exportFormat=both -Ddescriptor.generateHtml=true
