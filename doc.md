...
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:dependency-report
...
- name: Analyze Dependencies
  run: mvn io.github.tourem:deploy-manifest-plugin:3.0.0:analyze-dependencies
...
JSON + YAML + HTML :
```bash
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.exportFormat=both -Dmanifest.generateHtml=true
```

Arbre des dependances (Tree+Flat) avec scopes compile+runtime :
```bash
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.includeDependencyTree=true -Dmanifest.dependencyTreeFormat=both -Dmanifest.dependencyScopes=compile,runtime
```

Licences avec avertissements et liste incompatible personnalisee :
```bash
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.licenseWarnings=true -Dmanifest.incompatibleLicenses=GPL-3.0,AGPL-3.0,SSPL
```

Proprietes incluant les variables d’environnement :
```bash
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.includeProperties=true -Dmanifest.includeEnvironmentVariables=true
```

Plugins avec configuration et verification des mises a jour :
```bash
mvn io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.includePlugins=true -Dmanifest.includePluginConfiguration=true -Dmanifest.checkPluginUpdates=true -Dmanifest.generateHtml=true
```
...
- Guide d'intégration : `docs/INTEGRATION_STEP_BY_STEP.md`
- Documentation technique : `docs/YAML_CONFIG_SUMMARY.md`
...
- name: Generate Deploy Manifest
  run: mvn -B io.github.tourem:deploy-manifest-plugin:3.0.0:generate -Dmanifest.exportFormat=both -Dmanifest.generateHtml=true
